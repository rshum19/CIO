function [ Soln ] = OptCtrlSolver_wSlacks( OCP )
...
guess.slacks = interp1(IG.time',IG.slacks',guess.time')';

% Mux intial guess
[q0, packSize] = mux(guess.time,guess.state,guess.control,guess.lambda,guess.slacks);
...
slacks.lb = Bnds.slacks.lb*ones(1,nGrid);
lb = mux(time.lb, state.lb, ctrl.lb,lambda.lb,slacks.lb);
...
slacks.ub = Bnds.slacks.ub*ones(1,nGrid);
ub = mux(time.ub, state.ub, ctrl.ub,lambda.ub,slacks.ub);
...
[tSoln,xSoln,uSoln,lambdaSoln] = deMux(zSoln,packSize);
...
function [ q, packSize ] = mux(t,x,u,lambda,slacks)
...
lambda.Vec = reshape(z((3+nx+nu):(2+nx+nu+nl)),nLambda,nTime);
lambda.Xp = lambda.Vec(1:nCntcs,:);
lambda.Xn = lambda.Vec(nCntcs+1:2*nCntcs,:);
lambda.Y = lambda.Vec(2*nCntcs+1:end,:);
slacks = reshape(z((3+nx+nu+nl):(2+nx+nu+nl+ns)),nSlacks,nTime);
...
% Unmux state
[t,x,u,lambda,slacks] = deMux(z, packSize);
...
[f, Phi, Psi] = dynamics(t,x,u,lambda.Y);
...
if isempty(compCst)
    ...
    [c_comp, ceq_comp] = compCst(Phi,Psi,t,x,u,lambda,slacks);
end
