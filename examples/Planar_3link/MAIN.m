...
% State:
OCP.bounds.state.lb = [0; 0; -2*pi; 0; 0; -inf(3,1); -20; -20]; 
OCP.bounds.state.ub = [5; 10; 2*pi; pi; 2*pi/3; inf(3,1); 20; 20];

OCP.bounds.initState.lb = [0; 1.112; 0; 0; 0; zeros(5,1)];
OCP.bounds.initState.ub = [0; 1.112; 0; 0; 0; zeros(5,1)];

OCP.bounds.finalState.lb = [-2; 1; -2*pi; pi/3; pi/8; zeros(5,1)];
OCP.bounds.finalState.ub = [2; 2; 2*pi; pi/3; pi/8; zeros(5,1)];
...
% Save results
fileName = 'plana3link_soln';
overWrite = 1;
Notes = 'Solution found -- issue was that theta was constrained to be 0';
