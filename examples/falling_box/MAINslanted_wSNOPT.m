...
% -----------------------------------------------------------
OCP.Name = 'fallingBox_model';
OCP.model.params = params_fallingBox_model;
OCP.model.dynamics = @(t,x,u,lambda)fallingBox_slantedDyn_wrap(t,x,u,lambda,OCP.model.params);
...
OCP.ig.state = [OCP.ig.state(1:2,:); zeros(2,size(OCP.ig.state(1,:),2)); OCP.ig.state(3:4,:)];
...
% State:
OCP.bounds.state.lb = [-10*0; -50; -2*pi*0; -inf(3,1)]; 
OCP.bounds.state.ub = [10*0; 50; 2*pi*0; inf(3,1)];
OCP.bounds.initState.lb = [0; -50; 0; 0; 0; 0];
OCP.bounds.initState.ub = [0; 40; 0; 0; 0; 0];
OCP.bounds.finalState.lb = [-10*0; -50; -2*pi*0; -inf(3,1)];
OCP.bounds.finalState.ub = [20*0; 50; 2*pi*0; inf(3,1)];
...
for iter = 1:size(options,2)
    ...
    soln = OptCtrlSolver_wSNOPT_noU (OCP);
